    ...
    <div>
      <Header
        ...
        content={
          <HeaderContent
            avatar={
              distinct[0] ? null : event_list[0].userProfile_pic.pc1.toString()
            }
            title={distinct[0] ? null : event_list[0].username}
            para={distinct[0] ? null : event_list[0].eventType}
          />
        }
      />
      ...
      {formatlist.map((each) => {
        return (
          <List
            date={each.eventDate.slice(4, 10)}
            time={
              each.eventDate.slice(0, 3) + " " + each.eventDate.slice(16, 21)
            }
            location={each.eventLocation}
          />
        );
      })}
    </div>
